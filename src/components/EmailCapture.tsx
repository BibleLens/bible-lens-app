          ...
          <form onSubmit={handleSubmit} className="relative">
            ...
            <div className="flex gap-3">
              <input
                ...
                className="min-w-0 flex-1 px-4 py-2.5 rounded-none bg-[var(--color-bg-elevated)] border border-[var(--color-border)] text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--color-gold-400)]/50"
              />
              <button
                type="submit"
                disabled={status === "loading"}
                className="px-5 py-2.5 rounded-none bg-[var(--color-action-bg)] text-[var(--color-action-text)] font-medium hover:brightness-110 disabled:opacity-50 transition-all whitespace-nowrap"
              >
            ...
            {status === "error" && (
              <p className="text-[var(--color-error)] text-sm mt-2">{errorMessage}</p>
            )}
          </form>
        )}
      </div>
